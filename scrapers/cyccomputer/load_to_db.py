"""
Script para cargar productos de CycComputer desde JSON a la base de datos
"""
import json
import sys
import os
...
def load_cyccomputer_to_db():
    """Carga productos de CycComputer JSON a la base de datos"""
    
    print("\n" + "="*80)
    print("📥 CARGANDO PRODUCTOS DE CYCCOMPUTER A LA BASE DE DATOS")
    print("="*80 + "\n")
    
    # Inicializar base de datos (usar la BD principal en la raíz del proyecto)
    db_path = os.path.join(os.path.dirname(__file__), '..', '..', 'pc_prices.db')
    db = Database(db_path)
    db.init_db()
    ...
    try:
        ...
        print("   Ejecuta primero: cd scrapers/cyccomputer && python run.py")
        return False
    ...
    # Total
    cursor.execute("""
        SELECT COUNT(*) as count FROM products WHERE store = 'cyccomputer'
    """)
    total = cursor.fetchone()['count']
    print(f"\n📦 Total CycComputer en BD: {total}\n")
    
    conn.close()
    
    print("✅ Ahora puedes consultar los productos vía API:")
    print("   http://localhost:8000/api/stores/cyccomputer/products")
    print("   http://localhost:8000/api/stores/cyccomputer/stats\n")
    
    return True

if __name__ == "__main__":
    load_cyccomputer_to_db()
